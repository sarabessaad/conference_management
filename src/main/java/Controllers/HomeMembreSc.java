...
public class HomeMembreSc {
    ...
    private ItemMembreSc.MyListener myListener;
    ...
    void initialize() {
        ...
        nomUserProfile.setText(membreSc.getNom());

        ConfDataList.addAll(conferenceService.rechercheConfMembreSc(Login.id));
        System.out.println("load data");
        if (ConfDataList.size() > 0) {
            myListener = new ItemMembreSc.MyListener() {
                @Override
        ...
        for (int i = 0; i < ConfDataList.size(); i++) {
            try {
                FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/itemMembreSc.fxml"));
                AnchorPane anchorPane = fxmlLoader.load();
                ItemMembreSc item = fxmlLoader.getController();
