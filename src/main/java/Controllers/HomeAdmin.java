...
public class HomeAdmin {
    ...
    private ItemAdmin.MyListener myListener;

    @FXML
    void ajouter_conference(ActionEvent event) throws IOException {
        Parent page1 = FXMLLoader.load(getClass().getResource("/ajouterConf.fxml"));
    ...
    void initialize() {
        ...
        ConfDataList.addAll(conferenceService.rechercheConfPresident(Login.id));
        System.out.println("load data");
        if (ConfDataList.size() > 0) {
            myListener = new ItemAdmin.MyListener() {
                @Override
        ...
        for (int i = 0; i < ConfDataList.size(); i++) {
            try {
                ...
                ItemAdmin item = fxmlLoader.getController();
